...
describe Deliver::UploadScreenshots do
  describe "screenshot collection" do
    ...
    before do
      FakeFS::FileSystem.clone(File.join(Spaceship::ROOT, "lib", "assets", "deviceTypes.json"))
    ...
    it "should raise an error if unsupported screenshot sizes are in iMessage directory" do
      ...
      end.to raise_error(FastlaneCore::Interface::FastlaneError, "The Screenshot '/Screenshots/iMessage/en-GB/AppleTV-01First{3840x2160}.jpg' does not match the requirements of Apple-TV")
    end

    # TODO: Do we need to test in default dir?

    it "should detect screenshot from inside directories named by the device type" do
      add_screenshot("/Screenshots/en-GB/iphone65/01First{0x0}.jpg")
      screenshots = collect_screenshots_from_dir("/Screenshots/")
      expect(screenshots.count).to eq(1)
      expect(screenshots.first.device_type).to eq("iphone65")
    end
  end
end
